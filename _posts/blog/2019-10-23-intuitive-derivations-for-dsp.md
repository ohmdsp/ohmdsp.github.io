...
In practical computer applications, we do not use the DTFT (which has a continuous output) but instead use the DFT (which has a discrete output). For example, if we make our signal sample indices to be $$ 0 \leq n \leq N-1 $$ and uniformly sample the continuous output $$X(e^{j\omega})$$ at N equally spaced index points (or frequencies) between $$0 \leq k \leq N-1$$, then $$\omega = 2\pi k/N$$ and we get an expression for the DFT as
...
Again, we can simplify the right side to be
\$$  y[n] = z^{n} \sum^{\infty}_{k=-\infty}h[k]z^{-k} $$

Just like the previous experiment when we put in a complex exponential, we can see that the output is just the input multiplied by a complex scaling term. We can express this output as
\$$  y[n] = z^{n}H(z) $$
